    ...
    if (idx < n - window_size + 1) {
        ...
        // Update the hash value for the subsequent windows
         for (int i = 1; i < n - window_size + 1 - idx; ++i) {
            current_hash = (current_hash - s[idx + i - 1] * power[window_size - 1] % mod + mod) % mod;
...
extern "C" void processFASTA(const char* file_path, int window_size, int base, int mod) {
    ...
    for (const std::string& s : sequences) {
        ...
        std::vector<int> power(n + 1, 1);  // Precompute powers of base mod mod

        // Precompute powers of base mod mod (equivalent to what you did in the original code)
        for (int i = 1; i <= n; i++) {
